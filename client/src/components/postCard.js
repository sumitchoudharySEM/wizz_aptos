...
const PostCard = ({ setClicked, postid }) => {
  ...
  // const { mutate: execLike } = useSignAndExecuteTransactionBlock();
  ...
  // useEffect(() => {
  //   if (data) {
  //     //   console.log("Single Post Data", data.data.content.fields);
  //     setContent(data.data.content.fields.content);
  //     setOwner(data.data.content.fields.owner_profile_id);
  //     setLikes(data.data.content.fields.likes);
  //     setImgCID(data.data.content.fields.image);
  //   }
  // }, [data, error]);
  ...
  // useEffect(() => {
  //   if (ownerdata) {
  //     // console.log("Owner Data", ownerdata.data.content.fields);
  //     setOwnerName(ownerdata.data.content.fields.full_name);
  //     setOwnerUsername(ownerdata.data.content.fields.username);
  //     setownerImage(ownerdata.data.content.fields.profile_image_ref);
  //     setProfId(ownerdata.data.content.fields.profile_id);
  //   }
  ...
  return (
    ...
    hii
      {/* {Comments ? (
        <AddComment setClicked={ShowComments} />
      ) : (
        <div className={`w-full flex-col space-y-2 p-1 `}>
          <div className="py-2 flex space-x-2 align-middle justify-between">
            <Link href={`/profile/${profId}`}>
              <div className="flex space-x-4 align-middle">
                <div>
                  <img
                    className="rounded-full h-12 w-12"
                    src={`${NEXT_PUBLIC_GATEWAY_URL}/ipfs/${ownerImage}`}
                    alt=""
                  />
                </div>
                <div className="flex flex-col text-white justify-center">
                  <div className="text-xl font-medium">{ownerName}</div>
          ...
          <div className="text-white text-md">{content}</div>
          {imgCID !== null &&
          imgCID !== "" &&
          imgCID !== undefined &&
          imgCID !== "null" &&
          ...
          imgCID !== " " ? (
            <img
              ...
              src={`${NEXT_PUBLIC_GATEWAY_URL}/ipfs/${imgCID}`}
              alt=""
            />
          ) : (
            <></>
            ...
            <div className="flex">
              <button
                onClick={() => {
                  ...
                  const [like] = txb.moveCall({
                    target: `${PACKAGE_ID}::profilecontract::like_post`,
                    arguments: [txb.object(postid)],
                  });
                  execLike(
                    {
              ...
              >
                <div className="flex space-x-2 text-white  align-middle mr-8">
                  ...
                  <div className="text-[#ffffff] opacity-90">{likes}</div>
                </div>
              </button>
              <button
