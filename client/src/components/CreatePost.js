...
const CreatePost = ({ setClicked, userProfile }) => {
  ...
  const handleSubmission = async () => {
    try {
      if (selectedFile && selectedFile !== "undefined" && selectedFile !== "null" && selectedFile !== "" && selectedFile !==  null) {
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const transaction = {
                      data: {
                        function: `${MODULEADDRESS}::wizz::create_post`,
                        functionArguments: [
                          MODULEADDRESS,
                          content,
                          cid,
          ...
          <div>
            <div className=" w-full min-h-44 createpostwrapperimg">
              ...
              {/* <img
                   ...
                   alt=""
                 /> */}
                 {preview && preview.length > 0 && preview !== "undefined" && preview !== "null" && preview !== "" && preview !==  null ? 
              <img className="w-full rounded-2xl" src={preview} alt="hellow" />
              : <></>}
            </div>
          </div>
        </div>
