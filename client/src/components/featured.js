    ...
    try {
      ...
      while (counter <= taskCounter) {
        ...
        const ProfileResource = await aptos.getAccountResource({
          accountAddress: post.owner,
          resourceType: `${MODULEADDRESS}::wizz::Profile`,
        });
        Posts.push({
          owner: post.owner,
          ownerName: ProfileResource.full_name,
          ownerUsername: ProfileResource.username,
          ownerImage: ProfileResource.profile_image_ref,
          content: post.content,
          imgCID: post.image_ref,
          likes: post.likes,
        });
        counter++;
      }
    ...
    <>
      ...
      <div className="lg:w-[65%] md:w-[80%] w-[85%] flex flex-col align-middle my-3 h-full ">
        ...
        <div className="flex flex-col space-y-7">
            ...
            <Link href={`/profile/${item.owner}`}>
              <div className="flex space-x-4 align-middle">
                ...
                <div className="flex flex-col text-white justify-center">
                  <div className="text-xl font-medium">{item.ownerName}</div>
          ...
          {item.imgCID !== null &&
          item.imgCID !== "" &&
          item.imgCID !== undefined &&
          item.imgCID !== "null" &&
          item.imgCID !== "undefined" &&
          item.imgCID !== " " ? (
            <img
              className="h-auto w-full object-cover rounded-2xl"
              src={`${NEXT_PUBLIC_GATEWAY_URL}/ipfs/${item.imgCID}`}
              alt=""
            />
          ) : (
            <></>
